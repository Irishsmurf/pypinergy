...
class LoginResponse:
    ...
    def _from_dict(cls, d: dict) -> "LoginResponse":
        # Performance optimization: using `d.get("key") or {}` prevents allocating a
        # new empty dict/list default argument on every call when the key exists.
        # For list comprehensions, `or ()` is used because the empty tuple is a CPython singleton.
