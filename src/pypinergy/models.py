...
class LoginResponse:
    ...
    def _from_dict(cls, d: dict) -> "LoginResponse":
        ...
        # Performance optimization note: Using `d.get(key) or {}` instead of `d.get(key, {})`
        # and `(d.get(key) or ())` in comprehensions avoids unnecessary allocations and handles explicit None.
...
class ComparePeriod:
    ...
    def _from_dict(cls, d: dict) -> "ComparePeriod":
        # Performance optimization note: Using `d.get(key) or {}` avoids allocating a new dict
        # per evaluation on the happy path and guards against explicit None values.
        return cls(
            available=bool(d.get("available", False)),
