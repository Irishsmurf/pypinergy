...
def test_client_base_url_requires_https():
    with pytest.raises(
        ValueError, match="base_url must use https:// to prevent credential leakage"
    ):
        PinergyClient("user@example.com", "secret", base_url="http://example.com")

    client = PinergyClient(
        "user@example.com", "secret", base_url="http://localhost:8080"
    )
