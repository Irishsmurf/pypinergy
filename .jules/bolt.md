...
## 2025-03-02 - Avoiding requests dynamic header allocation overhead
**Learning:** `requests.Session` performance can be improved by pre-allocating persistent headers (e.g., `auth_token`) via `session.headers.update()` instead of passing them dynamically via `headers=` kwargs on every request. This avoids repetitive dictionary instantiation and merging. Using a property setter in `PinergyClient._auth_token` allows synchronizing the token securely into the session state without overhead.
**Action:** Always favor setting state in `session.headers` once rather than passing dynamic header dictionaries in tight IO/API wrapper loops.
