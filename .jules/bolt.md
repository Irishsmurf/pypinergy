...
## 2025-03-02 - Dataclass instantiation overhead in array parsing loops
**Learning:** For Python `< 3.10` data models that are heavily instantiated (like array items in time-series API responses), explicitly defining `__slots__` reduces memory usage by ~75% and speeds up instantiation by bypassing dynamic `__dict__` allocations. This is highly effective for large list responses where thousands of objects are generated in tight loops.
**Action:** Always add `__slots__` to internal API response dataclasses that are instantiated inside loops (like `UsageEntry` or `LevelPayDailyValue`) to significantly lower memory footprint and improve performance.
