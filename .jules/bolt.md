...
## 2025-03-02 - Mutable literals as dictionary get() defaults
**Learning:** In Python, using a literal collection like `d.get('key', [])` or `{}` creates a new memory allocation every time the expression is evaluated. When parsing nested objects in high throughput data APIs, this has a measurable performance and deep memory cost on the happy path.
**Action:** Use `d.get(key) or []` or `d.get(key) or {}` instead to short circuit when empty or use `(d.get(key) or ())` in comprehensions.
