...
## 2025-05-01 - Avoid mutable default allocations in dict.get()
**Learning:** Providing a mutable default like `[]` or `{}` to `dict.get()` allocates a new object every time the expression is evaluated. In hot loops parsing array data (like `UsageResponse._from_dict`), this adds measurable overhead.
**Action:** Replace `d.get("key", [])` with `d.get("key") or []` and `d.get("key", {})` with `d.get("key") or {}` to only allocate the fallback collection when the key is missing or falsy.
