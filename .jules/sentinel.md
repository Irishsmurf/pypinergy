...
## 2024-05-25 - [CRLF Injection via Unsanitized Header Input]
**Vulnerability:** The `check_email` method passes the user-provided `email` directly into an HTTP header (`email_address`). Unsanitized inputs allow an attacker to inject carriage returns (`\r`) and newlines (`\n`) to manipulate the request headers and potentially bypass security controls or perform HTTP Header Injection. Although HTTP clients like `requests` may inherently block CRLF by throwing a generic `InvalidHeader` exception, explicit sanitization at the application boundary is required to prevent unhandled crashes or confusing tracebacks.
**Learning:** Any user input destined for HTTP headers must be strictly sanitized, as standard library protections often translate into unhandled errors rather than safe execution.
**Prevention:** Always validate and sanitize user inputs before placing them into HTTP headers, explicitly rejecting `\r` and `\n` characters, and raising a clear application-level error (e.g. `ValueError`).
