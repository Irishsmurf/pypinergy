...
## 2024-05-05 - [CRLF Injection in Custom HTTP Headers]
**Vulnerability:** The `check_email` method accepted user input (`email`) and directly embedded it into a custom HTTP header (`email_address`) without sanitization. An attacker could provide an email containing Carriage Return and Line Feed (`\r\n`) characters, allowing them to inject arbitrary HTTP headers (CRLF injection) or manipulate the HTTP request.
**Learning:** Even though underlying HTTP libraries like `requests` may raise generic exceptions (e.g., `InvalidHeader`) when encountering CRLF characters in headers, explicit application-level validation is crucial. Relying on the underlying library can lead to unhandled exceptions crashing the application, bypassing intended error handling, or unexpected behaviors in different environments or library versions.
**Prevention:** Always validate and sanitize user input before embedding it into HTTP headers. Reject inputs containing `\r` or `\n` at the application boundary by raising explicit, handled errors like `ValueError` to ensure fail-safe execution and clear error semantics.
