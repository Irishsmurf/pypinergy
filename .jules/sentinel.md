...
## 2026-05-21 - CRLF Injection in HTTP Headers
**Vulnerability:** User input passed directly into HTTP headers without sanitization, leading to a CRLF injection vulnerability.
**Learning:** Even if the underlying HTTP library blocks CRLF injection by raising generic exceptions, it causes unhandled crashes or confusing tracebacks. Explicit sanitization is required at the application boundary.
**Prevention:** Always validate and sanitize user inputs that are used in HTTP headers to prevent CRLF injection.
