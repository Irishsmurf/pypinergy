...
## 2024-05-18 - Prevent CRLF Injection in Email Headers
**Vulnerability:** Unsanitized user input (`email`) passed directly to HTTP headers in `check_email` method allows for CRLF (HTTP Header Injection) vulnerabilities.
...
**Prevention:** Explicitly validate and sanitize any user inputs that are injected into HTTP headers. Reject inputs with `\r` and `\n` before they are sent to the network layer.
