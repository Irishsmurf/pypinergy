...
## 2024-05-24 - [CRLF / HTTP Header Injection via Unsanitized User Input in Custom Headers]
**Vulnerability:** The API client passed user-provided email addresses directly into custom HTTP headers (`email_address`) in the `check_email` method without sanitization. If an attacker provided an email containing `\r\n` (CRLF), they could inject malicious HTTP headers into the outgoing API request.
**Learning:** Even if underlying HTTP libraries like `requests` might inherently block CRLF injection by raising generic exceptions (e.g., `InvalidHeader`), explicit sanitization of user inputs used in headers is required at the application boundary (e.g., by raising `ValueError`). This prevents unhandled crashes, confusing tracebacks, and ensures the application explicitly defines its security boundaries.
**Prevention:** Always sanitize and validate user inputs before placing them into sensitive contexts like HTTP headers, especially when those inputs are expected to conform to simple formats like email addresses. Reject inputs containing `\r` or `\n` to prevent header injection.
