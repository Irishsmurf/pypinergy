...
## 2024-04-30 - [CRLF Injection via Unsanitized Headers]
**Vulnerability:** The API client passed user input (the `email` argument in `check_email`) directly into the `email_address` HTTP header. The underlying `requests` library eventually blocks raw CRLF injection in later versions by throwing an `InvalidHeader` exception, but passing unsanitized user input into headers at the application boundary risks unhandled exceptions (DoS) or successful HTTP Header Injection on earlier/different versions or if the HTTP client is swapped out.
**Learning:** Even if underlying HTTP libraries try to mitigate generic header injection, explicitly sanitizing user input destined for HTTP headers at the application level guarantees a safe failure mode (`ValueError`) and prevents confusing tracebacks or bypassing underlying library defenses.
**Prevention:** Always explicitly validate or sanitize any user input before assigning it as a value in custom HTTP headers, explicitly rejecting newline characters `\r` and `\n`.
