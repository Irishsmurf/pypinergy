...
## 2026-06-02 - HTTP Header Injection (CRLF) in `check_email`
**Vulnerability:** HTTP Header Injection via the `email_address` custom header.
**Learning:** `requests` automatically passes strings as custom HTTP headers without validating for carriage returns (`\r`) or newlines (`\n`). Attackers can manipulate these to inject custom headers.
**Prevention:** Sanitize user inputs passed directly to HTTP headers to reject `\r` and `\n` characters before sending the request.
