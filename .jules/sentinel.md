...
## 2026-05-12 - Prevent CRLF Injection in HTTP Headers
**Vulnerability:** User-controlled input (`email` in `check_email`) was directly passed to the `headers` dictionary in `requests` without sanitization. While `requests` may raise an internal `InvalidHeader` or `PinergyHTTPError`, it still attempts to process the headers, which could crash the application or expose tracebacks.
**Learning:** Even if underlying libraries like `requests` inherently block CRLF injection by raising generic exceptions, explicit sanitization of user inputs used in headers is required at the application boundary to fail safely and predictably with a `ValueError`.
**Prevention:** Always validate and reject `\r` and `\n` characters in any user-supplied string before passing it into HTTP headers, cookies, or logs.
