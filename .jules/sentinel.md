...
## 2024-04-17 - [HTTP Header Injection (CRLF) via Unsanitized Input]
**Vulnerability:** The `check_email` method accepted an unsanitized email address and placed it directly into the `email_address` HTTP header. An attacker could provide a payload containing `\r\n` (CRLF) characters, potentially injecting arbitrary HTTP headers into the outbound request, which may bypass security controls or alter backend parsing behavior.
**Learning:** Even if underlying libraries like `requests` might attempt to block CRLF injection internally, relying on them as the sole layer of defense is insecure. Explicit input validation at the application boundary is necessary, especially for data directly interpolated into custom headers.
**Prevention:** Always validate and sanitize user inputs before placing them into HTTP headers, file paths, or shell commands. Reject inputs containing `\r` or `\n` if they are destined for HTTP headers.
