...
## 2024-05-04 - [CRLF Injection via Unsanitized Custom HTTP Headers]
**Vulnerability:** The `check_email` method accepted an email address as a string and placed it directly into a custom HTTP header (`email_address`) without any validation or sanitization. Because user inputs passed into headers can contain carriage return and line feed characters (`\r` and `\n`), this created a CRLF (HTTP Header Injection) vulnerability.
**Learning:** Even if underlying HTTP libraries might handle or block some CRLF injections by raising generic exceptions, it is crucial to sanitize inputs meant for headers at the application boundary. Failing to do so can lead to unhandled exceptions, confusing tracebacks, or worse if the underlying implementation changes or the requests are crafted manually.
**Prevention:** Always validate and sanitize user inputs that are incorporated into HTTP headers to reject newline characters (e.g. raising a ValueError). Treat HTTP header parameters as high-risk execution vectors.
