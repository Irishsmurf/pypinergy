...
## 2024-05-24 - Fix CRLF Injection in email check header
**Vulnerability:** User-provided email string is passed directly to the `email_address` HTTP header without sanitization, risking CRLF injection.
**Learning:** Relying solely on `requests` implicit `InvalidHeader` exception is insufficient for security boundaries.
**Prevention:** Always validate and sanitize user input before passing it into custom HTTP headers, explicitly rejecting newlines (`\r`, `\n`) at the application layer.
