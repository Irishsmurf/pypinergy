...
## 2024-05-18 - Prevent CRLF HTTP Header Injection at Application Boundary
**Vulnerability:** User-controlled input (email_address) was directly passed to `check_email` and used as a raw HTTP header `{"email_address": email}`. An attacker could pass string payloads with `\r` and `\n` to inject arbitrary headers or split HTTP requests.
**Learning:** Even though underlying HTTP libraries like `requests` may inherently block CRLF injection by raising generic unhandled exceptions (e.g., `InvalidHeader`), the explicit application-boundary sanitization ensures inputs are explicitly validated and rejected cleanly (e.g., by raising `ValueError`) to prevent unhandled crashes, confusing stack traces, or potential logic flaws in HTTP adapters.
**Prevention:** Always explicitly validate and reject `\r` and `\n` characters in inputs that are mapped to HTTP headers at the application level, rather than relying solely on the underlying HTTP client implementation details.
