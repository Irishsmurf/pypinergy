...
## 2024-06-12 - [HTTP Header Injection via Custom Authentication Headers]
**Vulnerability:** The `check_email()` method in the API client accepted user-provided email strings and passed them directly into the `email_address` HTTP header without sanitization. An attacker could provide an email containing `\r\n` to inject arbitrary HTTP headers or manipulate the HTTP request structure.
**Learning:** The underlying `requests` library will eventually throw an `InvalidHeader` exception if it encounters newlines in header values. However, relying implicitly on underlying library exceptions at the transport layer for security validation is an anti-pattern. Validation should occur at the application boundary to explicitly reject malicious input and prevent unexpected behavior.
**Prevention:** Whenever user input is incorporated directly into custom HTTP headers, always explicitly validate and sanitize the input (e.g., rejecting `\r` and `\n`) before passing it to network libraries.
