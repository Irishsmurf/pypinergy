...
## 2024-03-29 - [Information Leakage via Unhandled JSON Decode Exceptions]
**Vulnerability:** The API client called `response.json()` directly on HTTP responses. If the API returned a non-JSON payload (e.g. an HTML error page from a gateway, or garbled response), `requests` raised a `JSONDecodeError` (which inherits from `ValueError`). This unhandled exception bubbled up to the application, exposing internal stack traces and implementation details.
**Learning:** Standard library helper methods like `response.json()` can raise parsing exceptions that leak underlying stack details. Whenever interacting with external APIs, expect the payload to occasionally fail parsing expectations and wrap parsing functions in `try...except` to sanitize exceptions and fail securely.
**Prevention:** In 'PinergyClient', JSON response parsing is encapsulated in a private helper `_parse_json(self, response: requests.Response) -> dict`. This helper handles invalid JSON by catching `ValueError` and raising a sanitized custom library exception `PinergyHTTPError("Failed to decode JSON response")`.
