...
## 2024-05-10 - [CRLF Header Injection via Unsanitized Input]
**Vulnerability:** The API client accepted unsanitized user inputs (like email addresses) and placed them directly into custom HTTP headers (`email_address`). Even though underlying HTTP libraries like `requests` may raise generic exceptions when encountering newline characters in headers, lacking explicit sanitization at the application boundary risks unhandled crashes or confusing tracebacks, bypassing intended validation flows.
**Learning:** Security validation must occur at the earliest possible boundary in the application logic. Depending on downstream dependencies to handle fundamentally invalid inputs can lead to unexpected behaviors or application state issues. User inputs passed to HTTP headers must always be explicitly sanitized to reject `\r` and `\n` characters to prevent CRLF injection vulnerabilities.
**Prevention:** Explicitly validate and reject any header inputs containing newline characters (e.g., `if "\r" in email or "\n" in email: raise ValueError(...)`) before making external HTTP calls.
