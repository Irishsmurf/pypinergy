...
## 2024-03-29 - [FIPS Compliance Crash via hashlib.sha1]
**Vulnerability:** The API client used `hashlib.sha1` without specifying `usedforsecurity=False` to generate password hashes. Since the external Pinergy API strictly mandates SHA-1, it cannot be upgraded to a secure algorithm. However, running this code in strict FIPS-compliant environments would result in a crash, as SHA-1 is considered an insecure hash algorithm.
**Learning:** When using deprecated or insecure hash algorithms (like MD5 or SHA-1) that are strictly required by third-party APIs or legacy systems, Python 3.9+ environments strictly enforcing FIPS will block execution unless you explicitly declare the hash is not used in a secure context.
**Prevention:** Always set `usedforsecurity=False` when calling `hashlib` functions for legacy or non-cryptographic purposes (like interacting with an API that forces an older hash). This makes the explicit choice obvious to readers and prevents crashes in strict runtime environments.
