...
## 2024-05-24 - [CRLF Injection via Unsanitized Custom Headers]
**Vulnerability:** The `check_email` method passed raw user input (email address) directly into a custom HTTP header (`email_address`) without sanitization. An attacker could inject Carriage Return Line Feed (`\r\n`) characters into the input, potentially allowing them to inject arbitrary HTTP headers (CRLF / HTTP Header Injection) into the outgoing request, depending on the behavior of the underlying HTTP client or proxies.
**Learning:** Even though underlying libraries like `requests` may inherently block CRLF injection by raising generic exceptions (e.g., `InvalidHeader`), explicit sanitization of user inputs used in HTTP headers is strictly required at the application boundary (e.g., by raising `ValueError`) to prevent unhandled crashes, confusing tracebacks, or subtle exploits if the HTTP client library is swapped or updated.
**Prevention:** Whenever user input is incorporated into an HTTP header, explicitly validate and sanitize the input to reject `\r` and `\n` characters before passing it to the HTTP library.
