...
## 2024-05-15 - [HTTP Header Injection (CRLF) via check_email]
**Vulnerability:** The `check_email` method passed user input directly into the `email_address` HTTP header without sanitizing newline characters. Although underlying libraries like `requests` may catch this and raise generic exceptions (e.g., `InvalidHeader`), it could still cause unhandled crashes or confusing tracebacks in applications, or potentially bypass weak library-level protections in other contexts.
**Learning:** Even if underlying HTTP libraries inherently block CRLF injection by raising generic exceptions, explicit sanitization of user inputs used in headers is required at the application boundary to prevent unhandled crashes or confusing tracebacks, ensuring consistent and secure failure modes.
**Prevention:** Always validate and sanitize user input against control characters (like `\r` and `\n`) before injecting them into HTTP headers, and raise explicit application-level errors (like `ValueError`) to handle them safely.
