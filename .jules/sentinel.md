...
## 2024-05-23 - Prevent CRLF HTTP Header Injection in check_email
**Vulnerability:** User inputs passed into HTTP headers (`email_address` in `check_email`) without explicit sanitization can lead to CRLF (HTTP Header Injection) vulnerabilities.
...
**Prevention:** Always validate and sanitize user inputs that are used as HTTP headers to reject `\r` and `\n` characters before passing them to the HTTP library.
