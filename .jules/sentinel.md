...
\n## 2024-05-11 - [HTTP Header Injection (CRLF) in `check_email`]
**Vulnerability:** The `check_email` method passed user-supplied `email` strings directly into the `email_address` HTTP header. If an attacker provided an email containing `\r` or `\n` characters, it could result in HTTP Header Injection (CRLF injection), bypassing intended application logic or causing unhandled exceptions if the underlying `requests` library crashed.
**Learning:** Even if underlying HTTP libraries like `requests` inherently block CRLF injection by raising generic exceptions (e.g., `InvalidHeader`), explicit sanitization of user inputs used in headers is required at the application boundary to prevent unhandled crashes or confusing tracebacks.
**Prevention:** Always sanitize and validate user input passed into HTTP headers. Explicitly reject or strip carriage returns and line feeds.
