...
## 2024-03-29 - [HTTP Header Injection / CRLF Vulnerability]
**Vulnerability:** The `check_email` method accepted an unfiltered string for the `email` argument and passed it directly into the `email_address` HTTP header of the request. Since `requests` does not reliably strip or encode Carriage Return (`\r`) and Line Feed (`\n`) characters in custom headers, an attacker could potentially inject new HTTP headers or split the HTTP request entirely, bypassing the intended request structure.
**Learning:** Never pass unvalidated user input directly into HTTP headers, even custom ones. Standard libraries like `requests` may raise `InvalidHeader` for some malformed inputs, but depending on the backend, proxy layer, or exact input string, CRLF sequences can still cause undefined behavior or injection vulnerabilities.
**Prevention:** Always validate and sanitize user input before placing it in an HTTP header. For string inputs like email addresses, explicitly check for and reject `\r` and `\n` characters before making the HTTP request.
