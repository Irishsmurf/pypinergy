...
## 2025-02-28 - [HTTP Header Injection via Unsanitized Email Input]
**Vulnerability:** The `check_email` method passed user-supplied `email` directly into an HTTP header without sanitization. An attacker could supply an email containing `\r\n` characters to inject arbitrary HTTP headers (CRLF injection) or manipulate the HTTP request structure.
**Learning:** Even if underlying HTTP libraries try to block some malicious headers by raising exceptions, an application must validate inputs at its own boundary to prevent unexpected errors or potential exploits, especially when inputs are used in HTTP headers.
**Prevention:** Always validate and sanitize user inputs that will be passed into HTTP headers. Reject inputs containing newline characters (`\r` or `\n`).
