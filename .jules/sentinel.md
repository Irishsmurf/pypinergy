...
## 2024-05-27 - [CRLF Injection via Unsanitized Headers]
**Vulnerability:** User input passed to HTTP headers (e.g., `email_address` in `check_email`) was not explicitly sanitized to reject `\r` and `\n` characters, which could lead to CRLF (HTTP Header Injection) vulnerabilities.
...
**Prevention:** Always sanitize inputs that are directly injected into HTTP headers by rejecting newline characters before passing them to the request mechanism.
