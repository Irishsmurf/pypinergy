...
## 2024-05-18 - [FIPS Compliance via Weak Hash Configuration]
**Vulnerability:** The application used `hashlib.sha1` without `usedforsecurity=False`. While the Pinergy API requires SHA-1 (which is cryptographically weak), calling `hashlib.sha1` natively in FIPS-compliant environments raises a `ValueError` by default if it believes it's being used for security.
**Learning:** When interacting with external APIs that require older, insecure hashing algorithms (like SHA-1 or MD5), you must explicitly signal to Python that it's an external requirement, not a security control. This is critical for compatibility with environments that enforce strict crypto boundaries.
**Prevention:** Whenever using weak hashing algorithms required by external integrations, always explicitly add `usedforsecurity=False` to prevent crashes in FIPS environments.
