...
## 2024-04-09 - [CRLF (HTTP Header Injection) in User Input]
**Vulnerability:** User inputs passed into HTTP headers (e.g., `email_address` in `check_email`) without explicit sanitization could lead to CRLF (HTTP Header Injection) vulnerabilities. While libraries like `requests` may catch and block invalid headers containing `\r` and `\n`, they typically raise generic exceptions (like `requests.exceptions.InvalidHeader`) which might be swallowed by broad `RequestException` catches and wrapped as generic HTTP errors.
**Learning:** Always sanitize user input meant for HTTP headers explicitly at the application boundary, even if the underlying HTTP library provides its own validation. Relying on library exceptions can mask the true nature of the error and cause confusing tracebacks or unhandled crashes.
**Prevention:** Explicitly reject `\r` and `\n` characters from any user input that will be interpolated into an HTTP header value, raising a clear validation error (e.g., `ValueError`) before the request is even constructed.
