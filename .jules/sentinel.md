...
## 2024-05-15 - [CRLF Injection via Unsanitized Headers]
**Vulnerability:** The `check_email` method accepted an unsanitized email address from the user and passed it directly into the `email_address` HTTP header. This is vulnerable to HTTP Header Injection (CRLF injection), as an attacker could supply an email containing `\r\n` characters to inject arbitrary headers or split the request, which `requests` might allow depending on the exact version/context.
**Learning:** Even when underlying libraries like `requests` might attempt to protect against HTTP header injection natively by raising an `InvalidHeader` exception, explicit application-level sanitization and validation is necessary to prevent application crashes, stack trace exposure, and ensure secure fail states at the application boundary rather than relying on an underlying library's potentially changing implementation.
**Prevention:** Always validate and sanitize user inputs that are injected into HTTP headers. Explicitly reject `\r` and `\n` characters at the boundary with a clean `ValueError`.
