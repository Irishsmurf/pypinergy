...
## 2024-04-28 - [HTTP Header Injection (CRLF) via Custom Header]
**Vulnerability:** The `check_email` method accepted an email parameter directly into a custom HTTP header (`email_address`). If `\r` and `\n` characters are used in the email address, they could bypass basic library-level injection protections or cause HTTP Header Injection since user input was unsanitized.
**Learning:** Even if underlying HTTP libraries like `requests` inherently block CRLF injection by raising generic exceptions, explicit sanitization of user inputs used in headers is required at the application boundary (e.g., by raising `ValueError`) to prevent unhandled crashes or confusing tracebacks. This is especially true for custom authentication headers.
**Prevention:** User inputs passed into HTTP headers (e.g., `email_address` in `check_email`) must be explicitly sanitized to reject `\r` and `\n` characters (e.g., raise `ValueError`).
