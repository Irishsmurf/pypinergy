...
## 2024-05-02 - [CRLF / Header Injection Vulnerability]
**Vulnerability:** The `check_email` method took a user-controlled `email` string and inserted it directly into the `email_address` HTTP header. This allowed HTTP Header Injection / CRLF injection if malicious newline characters (`\r` or `\n`) were provided in the email address.
**Learning:** Even if underlying HTTP libraries like `requests` inherently block CRLF injection by raising generic exceptions (e.g., `InvalidHeader`), explicit sanitization of user inputs used in headers is required at the application boundary (e.g., by raising `ValueError`) to prevent unhandled crashes or confusing tracebacks.
**Prevention:** Always validate and sanitize user inputs before placing them in HTTP headers. A simple check for `\r` and `\n` in the input can prevent Header Injection and potential upstream/downstream confusion.
