...
## 2024-04-24 - [Header Leakage via HTTP Header Injection (CRLF)]
**Vulnerability:** Unsanitized user inputs used directly in custom HTTP headers. In `check_email`, `email_address` is used as a header directly, allowing for injection of custom headers with `\r` and `\n` characters.
**Learning:** User inputs mapped to HTTP headers must be validated against CRLF characters `\r` and `\n` to prevent HTTP Header Injection vulnerabilities. The `requests` library blocks this under the hood, but explicitly validating inputs at the app boundary prevents unhandled errors or confusing tracebacks.
**Prevention:** Always sanitize any string that is passed to a `headers` dictionary by rejecting values with `\r` or `\n`.
