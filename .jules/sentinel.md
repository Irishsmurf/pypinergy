...
## 2024-04-19 - [CRLF Header Injection via Unsanitized User Input]
**Vulnerability:** User-provided data (`email`) was passed directly into a custom HTTP header (`email_address`) in the `check_email` method without sanitization. An attacker could include `\r\n` characters in the email string to inject arbitrary HTTP headers or manipulate the HTTP request.
...
**Prevention:** Always explicitly validate and sanitize any user-provided string before inserting it into an HTTP header. Reject any strings containing `\r` or `\n` at the application boundary.
