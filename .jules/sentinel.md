...
## 2025-01-01 - [FIPS Incompatibility in hashlib.sha1]
**Vulnerability:** The application used `hashlib.sha1` for password hashing to match external API requirements without marking it as non-security-critical. In strict FIPS 140-2 environments, using SHA-1 (which is cryptographically broken) without explicitly specifying `usedforsecurity=False` causes Python to raise a `ValueError`, immediately crashing the authentication flow.
**Learning:** Legacy APIs frequently require outdated cryptographic primitives (like MD5 or SHA-1) for interoperability rather than security. Python's `hashlib` is increasingly strict about invoking these algorithms in environments where FIPS mode is enabled at the OS level (e.g. RedHat/Fedora derivatives).
**Prevention:** Whenever hashing functions like `md5` or `sha1` are used strictly for non-security checksumming or API compliance, explicitly pass `usedforsecurity=False` in the `hashlib` call. This signals to the runtime that the usage is intentional and safe, avoiding unexpected runtime crashes in restricted environments.
