...
## 2024-03-19 - [Credential Leakage via Plaintext HTTP]
**Vulnerability:** The `PinergyClient` allowed arbitrary `base_url` overrides (useful for testing/proxies) without enforcing HTTPS. If a user was tricked or misconfigured their client to point to a plaintext `http://` endpoint (other than localhost), their credentials (email and SHA-1 password hash) would be transmitted in plaintext and easily interceptable.
**Learning:** Any API client transmitting sensitive credentials must enforce secure transport (HTTPS) at the application level. While local testing environments (`localhost`, `127.0.0.1`) might be exempted for developer convenience, the validation must be strict using proper URL parsing (like `urllib.parse.urlparse`) to prevent bypasses like `http://localhost.example.com`.
**Prevention:** Always validate URL schemes before transmitting credentials. Hardcode or enforce `https://` for remote hosts and raise a clear error when an insecure protocol is used.
