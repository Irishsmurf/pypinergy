...
## 2024-03-22 - [Credential Leakage via Insecure Base URL]
**Vulnerability:** The `PinergyClient` allowed arbitrary `http://` URLs for its `base_url`. Since the client sends sensitive custom headers (e.g., `auth_token`, and the hashed password during login), an attacker or misconfiguration could force the client to send these credentials over plaintext HTTP, exposing them to interception.
**Learning:** Client libraries that handle authentication and sensitive data must actively prevent insecure usage, even if the primary API uses HTTPS. Users may mistakenly configure an `http://` base URL or be targeted by downgrade attacks.
**Prevention:** Explicitly validate that user-provided endpoints strictly use `https://` schemes. Provide narrow, strictly validated exceptions only for local development (`localhost`, `127.0.0.1`) using `urllib.parse.urlparse` to prevent bypasses like `http://localhost.example.com`.
