...
## 2024-05-06 - [HTTP Header Injection / CRLF Injection via Unsanitized Email]
**Vulnerability:** The API client passed user input (`email`) directly into an HTTP header (`email_address`) in the `check_email` method without checking for carriage return (`\r`) or line feed (`\n`) characters.
...
**Prevention:** Always validate and sanitize user inputs that are used in HTTP headers, explicitly rejecting `\r` and `\n` characters before passing them to the HTTP client.
