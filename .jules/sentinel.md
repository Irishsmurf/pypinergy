...
## 2024-04-10 - [CRLF Injection in HTTP Headers via User Input]
**Vulnerability:** The API client passed the unsanitized `email_address` input directly into the `headers` dictionary during the `check_email` request. A malicious user could inject carriage return (`\r`) and line feed (`\n`) characters to terminate the header block early and inject arbitrary headers or an entirely new HTTP response body.
**Learning:** Even though underlying HTTP libraries like `requests` may inherently block CRLF injection by raising generic low-level exceptions (e.g. `InvalidHeader`), relying on this behavior is insecure and brittle. It leaks internal implementation details via tracebacks and may crash the application unexpectedly. Explicit sanitization of user inputs used in headers is required at the application boundary.
**Prevention:** Always sanitize or strictly validate user inputs that are interpolated into HTTP headers or request paths. Explicitly reject `\r` and `\n` characters before passing the data to the HTTP client to ensure a secure, handled failure state (like a custom `ValueError`).
