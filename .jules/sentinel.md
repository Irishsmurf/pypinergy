...
## 2024-06-01 - [HTTP Header Injection via Unsanitized Input]
**Vulnerability:** The `check_email` method accepted an unsanitized email address and passed it directly into a custom HTTP header (`email_address`). If an attacker provided an email containing newline characters (`\r` or `\n`), it could cause HTTP header injection. While `requests`'s `InvalidHeader` implicitly catches this in newer versions, it is critical to sanitize input at the application boundary rather than relying on underlying libraries to fail.
**Learning:** Never pass raw user input into HTTP headers without explicitly validating or sanitizing it at the application boundary, even if underlying HTTP libraries might eventually catch it with a generic exception. Relying on implicit exceptions from libraries is not sufficient for robust application security.
**Prevention:** Always explicitly check for and reject newline characters (`\r`, `\n`) in any string that will be used as a header value, raising a clear, application-specific exception like `ValueError` before making the network call.
